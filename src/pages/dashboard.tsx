...
const DashboardPage: React.FC = () => {
  ...
  if (!user && !isLoading) {
    ...
    return <Navigate to="/login" replace state={{ from: `/events/${eventId}` }} />;
  }
    ...
    return (
      <div className="bg-yellow-50 p-4 mb-4 rounded border border-yellow-300">
        ...
        <div className="text-sm">
          ...
          <div><strong>Event ID:</strong> {eventId || 'null'}</div>
          <div><strong>Investor:</strong> {investor ? `${investor.id} (${investor.name})` : 'null'}</div>
                ...
                <div className="flex">
                  ...
                  <div className="ml-3">
                    <p className="text-sm text-yellow-700">
                      You are viewing this event in read-only mode. Contact an administrator to become an investor.
                    </p>
                  </div>
                </div>
              </div>
