...
export const FounderInvitationManager: React.FC<
	FounderInvitationManagerProps
> = ({ events, onInvitationsSent, className = "" }) => {
	const { user } = useAuth();
	...
	const handleSendInvitations = async () => {
		...
		try {
			...
			if (!data || data.length === 0) {
				throw new Error(
					"No invitations were created. Check database permissions."
				);
			}

			console.log("Successfully created invitations:", data);
			setSuccessMessage(
				`Successfully created ${data.length} invitation(s). Copy the links below to send via email.`
			);
			setCreatedInvitations(
				data.map((inv) => ({ email: inv.email, token: inv.invitation_token }))
			);
			setEmailList("");
	...
	return (
		<div
			className={`bg-white rounded-lg shadow-md p-6 ${className} text-black`}
		>
				...
				{createdInvitations.length > 0 && (
					<div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
						...
						<p className="text-sm text-blue-700 mt-3">
							💡 Copy these links and send them to the founders via email. Each
