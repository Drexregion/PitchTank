...
interface ConversationsPanelProps {
	...
	suggestionsEventId?: string | null;
}
...
export const ConversationsPanel: React.FC<ConversationsPanelProps> = ({
	...
	const fetchRecommendations = useCallback(async (force = false) => {
		...
		if ((!stale && !force) || !suggestionsEventId || !userId) return;
		...
		try {
			...
			const res = await fetch(`${supabaseUrl}/functions/v1/recommend-connections`, {
				...
				body: JSON.stringify({ eventId: suggestionsEventId }),
			});
			if (res.ok) {
	...
	}, [isOpen, userId, loadThreads, fetchRecommendations]);
				...
				>
					...
					<div className="relative z-10 flex flex-col flex-1 min-h-0">
						...
						<div className="flex-1 min-h-0 overflow-y-auto">
							{/* Public channel row — only when a public chat handler is provided */}
							{onOpenPublicChat && (
								<button
									...
									{(publicOnlineCount ?? 0) > 0 && (
										...
										>
											...
											<span className="text-green-400 text-[10px] font-bold tabular-nums">
												{(publicOnlineCount ?? 0) >= 1000
													? `${((publicOnlineCount ?? 0) / 1000).toFixed(1)}k`
													: publicOnlineCount}
											</span>
										</div>
									)}
							...
							{/* Suggested connections — only when inside an event context */}
							{suggestionsEventId && (
								<div>
