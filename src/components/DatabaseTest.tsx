...
export const DatabaseTest: React.FC = () => {
	...
	const testDatabaseConnection = async () => {
		...
		try {
			...
			if (eventsError) {
				setTestResult(
					(prev) => prev + `❌ Failed to fetch events: ${eventsError.message}\n`
				);
				return;
			}
			...
			if (!events || events.length === 0) {
				setTestResult(
					(prev) => prev + `❌ No events found. Please create an event first.\n`
				);
				return;
			}
			...
			setTestResult(
				(prev) => prev + `✅ Found event: ${events[0].name} (${eventId})\n`
			);
			...
			setTestResult(
				(prev) => prev + "🔄 Attempting to insert test invitation...\n"
			);
			...
			if (error) {
				...
				setTestResult(
					(prev) => prev + `Created record: ${JSON.stringify(data, null, 2)}\n`
				);
			}
			...
			if (readError) {
				setTestResult(
					(prev) => prev + `❌ Read failed: ${readError.message}\n`
				);
			} else {
				setTestResult(
					(prev) =>
						prev +
						`✅ Read successful! Found ${readData?.length || 0} records\n`
				);
			}
			...
			if (eventsReadError) {
				setTestResult(
					(prev) => prev + `❌ Events read failed: ${eventsReadError.message}\n`
				);
			} else {
				setTestResult(
					(prev) =>
						prev +
						`✅ Events read successful! Found ${
							eventsData?.length || 0
						} events\n`
				);
				if (eventsData && eventsData.length > 0) {
					setTestResult(
						(prev) =>
							prev + `Events: ${eventsData.map((e) => e.name).join(", ")}\n`
					);
				}
			}
		} catch (err: any) {
